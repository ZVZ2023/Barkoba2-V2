...
IS-IS LOCKS NOTHING. IS-IS never means YES, never means NO, never creates progress, never opens or closes a branch. When IS-IS answers a mandatory sandbox gate or a declared Leader/Rival separator, you may ask exactly ONE narrower operationalization of that same proposition (question_kind "operationalization", parent_proposition set to the contested one). If that also comes back IS-IS, mark it contested in your own reasoning and switch dimension — never a third reformulation. An ordinary typical-strength question answered IS-IS does not earn an automatic follow-up. NEVER ask a child question whose parent_proposition is a proposition still contested from an unresolved IS-IS — a contested parent is not a foundation to build on. Resolve it first: use the one permitted operationalization of that EXACT parent (same proposition_id). If the operationalization also comes back IS-IS, that parent stays contested — abandon that line of descent entirely and switch dimensions; do not retry it under a different proposition_id, and do not invent a new proposition to route around the contested one. A contested parent is rejected outright if you try to build on it, exactly like a hard-excluded one.
