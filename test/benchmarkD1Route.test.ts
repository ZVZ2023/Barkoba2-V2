...
import { POST as runD1Route } from "../app/api/internal/benchmark/d1-generic-backpack/route";

// ---------------------------------------------------------------------------
// TEMPORARY route, temporary test file — both belong to the M1 D-1
// proof-of-concept and should be deleted together once the run is verified.
//
// Covers the CURRENT gate model (post self-HTTP-removal, in-process runner):
//   1. Preview-only (VERCEL_ENV === "preview").
//   2. Exact confirmation body {"confirm":"run-d1-once"}, no other key/value.
...
//      log line.
//
// runD1Fixture() now runs entirely in-process (createSecret/createGame/
// saveGame use the in-memory KV fallback automatically active whenever
...
// exercised from this in-process test (there is no real HTTP edge here) —
// that boundary is verified operationally, not by this file. What this file
// verifies is everything the route's OWN code is responsible for.
// ---------------------------------------------------------------------------
...
function request(body?: unknown): Request {
  return new Request("https://preview.test/api/internal/benchmark/d1-generic-backpack", {
    method: "POST",
...
test("GATE 1: refuses with 404 when VERCEL_ENV is unset (e.g. local dev), even with a correct confirmation", async () => {
  ...
  const res = await runD1Route(request({ confirm: "run-d1-once" }) as any);
...
test("GATE 2: refuses with 400 in Preview when the body is empty", async () => {
  ...
  process.env.BENCHMARK_INGRESS_SECRET = TEST_SECRET;
  const res = await runD1Route(request() as any);
...
test("GATE 2: refuses with 400 in Preview when the body is not valid JSON", async () => {
  process.env.VERCEL_ENV = "preview";
  process.env.BENCHMARK_INGRESS_SECRET = TEST_SECRET;
  const badReq = new Request("https://preview.test/api/internal/benchmark/d1-generic-backpack", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: "not json",
  });
  const res = await runD1Route(badReq as any);
  assert.equal(res.status, 400);
  assert.equal((await res.json()).error, "confirmation_required");
});
...
test("GATE 2: refuses with 400 in Preview when confirm has the wrong value", async () => {
  process.env.VERCEL_ENV = "preview";
  process.env.BENCHMARK_INGRESS_SECRET = TEST_SECRET;
  const res = await runD1Route(request({ confirm: "please" }) as any);
  assert.equal(res.status, 400);
  assert.equal((await res.json()).error, "confirmation_required");
});
...
test("GATE 2: refuses with 400 in Preview when an extra key is present alongside a correct confirm", async () => {
  process.env.VERCEL_ENV = "preview";
  process.env.BENCHMARK_INGRESS_SECRET = TEST_SECRET;
  const res = await runD1Route(
    request({ confirm: "run-d1-once", target: "something else" }) as any
  );
  assert.equal(res.status, 400);
  assert.equal((await res.json()).error, "confirmation_required");
});
...
test("GATE 2 cleared: an exact confirmation body in Preview with a configured secret actually attempts the in-process run", async () => {
  ...
  // runD1Fixture() at all (past both gates and the readiness check), without
...
// ---------------------------------------------------------------------------
// No-secret-leakage — exercised on a real failure path through runD1Fixture,
...
test("LEAKAGE: neither the response body nor anything logged ever contains the real secret value", async () => {
  process.env.VERCEL_ENV = "preview";
  process.env.BENCHMARK_INGRESS_SECRET = TEST_SECRET;
  // The fake ANTHROPIC_API_KEY from beforeEach forces runD1Fixture to log its
  ...
  const { result: res, logged } = await captureConsole(() =>
    runD1Route(request({ confirm: "run-d1-once" }) as any)
  );
