...
test("SOURCE: the ai_composer creation branch records racer_player_id from the resolved caller", () => {
  ...
  // V2.8.8 — widened from 4000: the mode-resolution block added ahead of
  // racer_player_id in this branch pushed it past the old fixed window.
  const aiComposerBranch = CREATE_ROUTE_SRC.slice(aiComposerBranchStart, aiComposerBranchStart + 5000);
  assert.match(
    aiComposerBranch,
