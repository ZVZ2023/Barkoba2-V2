...
import { POST as runD3Route } from "../app/api/internal/benchmark/heldout-01-mona-lisa/route";

// ---------------------------------------------------------------------------
// TEMPORARY route, temporary test file — both belong to the M4 held-out
// benchmark case and should be deleted together once the run is verified.
// Mirrors test/benchmarkD2Route.test.ts exactly, pointed at the held-out
// route and its "run-heldout-01-once" confirmation string — see that file's
...
//   2. Exact confirmation body {"confirm":"run-heldout-01-once"}, no other
//      key/value.
...
//      log line.
//
// runD3Fixture() runs entirely in-process (createSecret/createGame/saveGame
...
// and none should be. This test file does not run the held-out fixture for
// real.
//
...
function request(body?: unknown): Request {
  return new Request("https://preview.test/api/internal/benchmark/heldout-01-mona-lisa", {
    method: "POST",
...
test("GATE 1: refuses with 404 when VERCEL_ENV is unset (e.g. local dev), even with a correct confirmation", async () => {
  ...
  const res = await runD3Route(request({ confirm: "run-heldout-01-once" }) as any);
...
test("GATE 2: refuses with 400 in Preview when the body is empty", async () => {
  ...
  process.env.BENCHMARK_INGRESS_SECRET = TEST_SECRET;
  const res = await runD3Route(request() as any);
...
test("GATE 2: refuses with 400 in Preview when the body is not valid JSON", async () => {
  process.env.VERCEL_ENV = "preview";
  process.env.BENCHMARK_INGRESS_SECRET = TEST_SECRET;
  const badReq = new Request("https://preview.test/api/internal/benchmark/heldout-01-mona-lisa", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: "not json",
  });
  const res = await runD3Route(badReq as any);
  assert.equal(res.status, 400);
  assert.equal((await res.json()).error, "confirmation_required");
});
...
test("GATE 2: refuses with 400 in Preview when confirm has the wrong value", async () => {
  process.env.VERCEL_ENV = "preview";
  process.env.BENCHMARK_INGRESS_SECRET = TEST_SECRET;
  const res = await runD3Route(request({ confirm: "please" }) as any);
  assert.equal(res.status, 400);
  assert.equal((await res.json()).error, "confirmation_required");
});

test("GATE 2: refuses with 400 in Preview when the D-1 or D-2 confirmation string is used instead of this fixture's own", async () => {
  // No two fixture routes must ever be triggerable by each other's
  // confirmation body — this is the entire reason the strings differ.
  process.env.VERCEL_ENV = "preview";
  process.env.BENCHMARK_INGRESS_SECRET = TEST_SECRET;
  for (const wrongConfirm of ["run-d1-once", "run-d2-once"]) {
    const res = await runD3Route(request({ confirm: wrongConfirm }) as any);
    assert.equal(res.status, 400);
    assert.equal((await res.json()).error, "confirmation_required");
  }
});
...
test("GATE 2: refuses with 400 in Preview when an extra key is present alongside a correct confirm", async () => {
  process.env.VERCEL_ENV = "preview";
  process.env.BENCHMARK_INGRESS_SECRET = TEST_SECRET;
  const res = await runD3Route(
    request({ confirm: "run-heldout-01-once", target: "something else" }) as any
  );
  assert.equal(res.status, 400);
  assert.equal((await res.json()).error, "confirmation_required");
});
...
test("GATE 2 cleared: an exact confirmation body in Preview with a configured secret actually attempts the in-process run", async () => {
  ...
  // runD3Fixture() at all (past both gates and the readiness check), without
...
// ---------------------------------------------------------------------------
// No-secret-leakage — exercised on a real failure path through runD3Fixture,
...
test("LEAKAGE: neither the response body nor anything logged ever contains the real secret value", async () => {
  process.env.VERCEL_ENV = "preview";
  process.env.BENCHMARK_INGRESS_SECRET = TEST_SECRET;
  // The fake ANTHROPIC_API_KEY from beforeEach forces runD3Fixture to log its
  ...
  const { result: res, logged } = await captureConsole(() =>
    runD3Route(request({ confirm: "run-heldout-01-once" }) as any)
  );
