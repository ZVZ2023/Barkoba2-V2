...
### 51.9 M2 — production test evidence closes two remaining UX failure points

Production testing on `barkobak.com` confirmed the newcomer path worked end to end: anonymous game → registration → Resend email → `/verify-email` → verified account → 5 additional Play Credits. Two failure points remained, both fixed as the smallest safe change against that working path.

**No more recovery-code stop after verification.** `POST /api/account/verify-email` no longer calls a rotate-and-generate step at all — it calls the plain, already-idempotent `markEmailVerified()` and returns straight to a welcome/CTA screen. The atomic `markEmailVerifiedAndRotateRecoveryKey` added for §51.8's scanner-safety fix is removed entirely: nothing calls it any more, and code that generates a secret nobody is shown is worse than no code, not neutral. **Recovery capability is not deleted** — it was never *created* by that function in the first place, only ever displayed by it; the underlying credential and its only exposed generator (`POST /api/account/rotate-recovery-code`, reachable from Profil via ClaimPrompt's `protected` branch) are completely unchanged. A freshly verified player's registration-time recovery key stays exactly as unseen as ClaimPrompt already made it (§51.8's own prior change) — reachable on demand, later, at the player's own initiative, which is where a backup credential belongs.
...
### 52 V2.8.0 — one public "Barkóba AI", server-authoritative; product version split from guidance version

**Product packaging, not a Racer-intelligence change.** `racer/4.0.0` (`CORE_RACER_RULES` in `lib/prompts/racer.ts`) is byte-identical to what it was before this release. The rejected candidate-validation-gate experiment and the lab-only required-target-grain experiment (both explored on the separate `grok-baseline-v2.8` branch, never merged) remain absent from this release path entirely — this section documents packaging and provenance work only.

**What changed.** Ordinary players choosing a human-Composer game (`/compose`, `app/ComposerEntry.tsx`) used to see a visible "Ki találgasson?" (who should guess?) picker offering "Claude" or "Grok" by name, sent verbatim as `racer_provider` in the game-creation request. That picker and the client-side state behind it are removed entirely — the field is no longer sent at all, not merely defaulted.

**Server authority, not UI hiding alone.** `app/api/game/create/route.ts` gained `PUBLIC_RACER_PROVIDER: ModelProviderId = "xai"` — a product-policy constant, deliberately separate from `DEFAULT_RACER_PROVIDER` (which stays `"anthropic"` and untouched; it is a transport-layer fallback for callers that supply no provider argument at all, not a statement about what ordinary players get). For every caller *except* one that already passed `resolveBenchmark()`'s existing secret-header gate, the value fed into `resolveRacerProvider()` is now `PUBLIC_RACER_PROVIDER`, never `body.racer_provider` — so no request-body override, present or future, can move an ordinary game onto a different provider. `resolveRacerProvider()`'s own two-refusals-never-a-substitution contract (400 `unknown_provider` / 503 `provider_unavailable`) is completely unchanged; only which value reaches it changed. An admin/lab/benchmark caller presenting the same secret header this route already required for benchmark tagging may still propose a provider, exactly as before — one existing gate now does double duty (tags the game AND authorizes provider choice), rather than a second one being invented.

**No silent cross-provider fallback exists anywhere in this codebase, audited before this release and unaffected by it.** `getAdapter()`, `isProviderAvailable()`, `resolveRacerProvider()`, the turn route's per-turn provider re-read from `game.racer_provider` (never from the request), and `xai.ts`'s own missing-key check all refuse outright (throw or 400/503/502) rather than substitute. This was true before V2.8.0 and remains true after it — see `test/xaiProvider.test.ts`'s existing source-structure guards and the new `test/publicRacerAuthority.test.ts`.

**Model identity is a runtime/environment concern, deliberately not re-decided here.** `env.xaiModelRacer()`'s code-level fallback stays `"grok-4.6"` — `test/versionVisibility.test.ts`'s own stated reasoning ("switched on by SETTING XAI_MODEL_RACER, never by moving the default... a future deploy without the variable must behave as it always has") governs, and this release respects it rather than overriding it. Reaching a specific model (including `grok-4.20-0309-reasoning`) is entirely a matter of what `XAI_MODEL_RACER` is set to in the deployment environment — a production configuration decision, out of scope for this code change, and reported separately as the exact production-only action remaining (see the V2.8.0 release report itself, not repeated here since environment values drift and this document should not need editing every time they do).

**Selection-mode provenance needs no new storage.** Whether a game's provider was chosen by the ordinary public path or by an admin/benchmark caller is already a deterministic function of two fields already durable in `corpus.games`: `benchmark_case_id` (non-null ⇒ internal/admin/benchmark caller, since that is the only way to set it) and `racer_provider` (set ⇒ an AI raced at all). No migration, no new column, no duplicate storage was added for this — reconstructable from what already exists.

**The operating principle this release exists to serve, stated once so it does not need restating per-section:**

> Every game is evidence.
> Some observations become lessons.
> Some lessons earn promotion.

Ordinary Human↔AI games (now all one coherent `xai`/`racer/4.0.0` population under the public path, rather than silently mixing whichever provider a player happened to click) feed Experience Memory — the durable corpus (`corpus.games`, `corpus.game_turns`, `corpus.game_targets`, `corpus.game_resolutions`, `corpus.game_corrections`) that already captures target, full question/answer/AMBIGUOUS history, corrections, final guess, result, provider, exact model, guidance version, and question budget for every game with at least one answered question — see `lib/corpus/gameCorpus.ts`. Controlled, pre-registered experiments (like the two named above) remain the only gate through which an observation becomes a promoted Strategy Memory entry; ordinary play observes, it does not promote.

### 53 V2.8.1 — My Car Key integrity hotfix: exact answer-to-question binding

**A game-state integrity fix, not a latency fix.** Production incident, target "My Car Key" (2026-09-01): a human's answer landed on a question they never saw, and the game was left stranded. Forensic reconstruction (this session's own read-only investigation, cross-checked against Vercel's runtime-error log — three genuine 60-second `Task timed out` errors on `/api/game/[id]/turn` inside the incident window, on the exact production deployment) found the mechanism: `POST /api/game/[id]/turn` accepted an answer with no binding to the exact question the client believed it was answering, and no compare-and-swap on the underlying write. A client retry issued while an earlier, slow request was still completing server-side (or had already completed without the client successfully receiving that response) could have its answer applied to whatever question the server had *since* advanced to — a stale answer landing on a live one. The confirmed 60-second timeouts are a real, separate latency problem (`grok-4.20-0309-reasoning` has no documented latency probe anywhere in this codebase, unlike `grok-4.6`'s — see §29's GROK-01/02/03 cluster) and are explicitly **not** addressed here; this release protects game-state integrity *even when* those timeouts keep happening, which is the point of fixing the two separately.

**The invariant.** A human answer must never be applied to a question unless the server can verify it was submitted for the exact pending question/state it targets. A stale, retried, duplicated, or concurrent request must never answer a later question.

**Mechanism.** `GameRecord` gains `revision` (`lib/types.ts`) — a monotonic counter with its own KV key, always freshly re-read on every `getGame()`, never trusted from whatever was last baked into the stored blob. `lib/kv.ts` gains two primitives narrow enough to need nothing beyond what any Redis-compatible backend already guarantees: `acquireLock`/`releaseLock` (native `SET key val NX EX`, one atomic command) and `casSetWithRevision` (one Lua script using only `redis.call('GET'|'SET', ...)` — no `cjson`, no external library — so it runs unmodified against Upstash's REST-backed `EVAL`). `POST /turn` now requires `expected_revision` on every answer submission; there is no compatibility fallback to the old unsafe behavior (audited: `GameClient.tsx` is the only caller of this route for the human-Composer/AI-Racer path). A mismatch is rejected — `409 stale_turn`, canonical current state attached — before any mutation and before any Racer call is ever made. A short-lived per-game advisory lock (TTL matched to the route's own 60-second `maxDuration`) serializes genuinely concurrent requests, so the losing one is rejected before it can pay for an LLM call at all; the revision CAS remains the actual data-integrity guarantee regardless. `POST /correct` adopts the same lock and CAS save — its own `expected_log_length` validation is unchanged, but the underlying write is no longer a blind overwrite that could race an in-flight `/turn`, closing a gap the hotfix would otherwise have left open at the storage layer between the two routes that can mutate a live game.

**What this does not claim.** Not a latency fix, not a Grok/model change, not a change to `racer/4.0.0`, adjudication, or IS-IS semantics. The exact-duplicate-question guard prepared earlier remains parked on its own branch, untouched and not part of this release.

**Validated end-to-end, not only unit-level.** `test/turnIntegrityHotfix.test.ts` drives the real `POST` handlers directly (no HTTP server, but the actual exported route functions, the actual `lib/gameStore.ts`, the actual in-memory KV fallback — only the Racer LLM call itself is mocked) through: the golden My Car Key reproduction (a completed request followed by a stale retry targeting the question it superseded), true concurrency (two requests racing the same pending question, confirmed via explicit polling rather than an assumed execution order — Node's `await`-yields-a-microtask-per-step semantics make "runs to completion before the next call starts" a wrong assumption even against a fully synchronous in-memory store), a `/turn`-vs-`/correct` race, and ordinary YES/NO/AMBIGUOUS and correction regression. Building this surfaced a real, if dev-only, latent bug along the way: `InMemoryKV` (the local-dev/test KV fallback) was returning shared mutable object references from reads instead of independent copies, unlike real Upstash, which always deserializes fresh over HTTP — fixed with `structuredClone` on both the read and write paths, so the fallback actually matches the isolation guarantee real infrastructure provides rather than occasionally providing a stronger one no test should be allowed to rely on.

### 54 V2.8.2 — exact-duplicate question guard, promoted onto the integrity hotfix

**Promotion, not new design.** The exact-duplicate-question pre-emission guard (`lib/duplicateQuestionGuard.ts`) was already fully specified, implemented, and validated on its own experimental branch — Marcali's 3 known exact duplicates caught, zero false fires on Black hole/Hole, the Q31 near-duplicate correctly not firing, a Preview forced-duplicate regeneration proof. None of that was redone here; this release is the mechanical act of landing it on top of §53's integrity hotfix, which had since rewritten the same route.

**The merge.** `/turn`'s Steps 2–4 (spend check, Racer call, Guess Detector) had been extracted into `runOneRacerAttempt` by the duplicate-guard branch so the guard could retry it; §53 had separately wrapped the whole turn-generation section in a per-game lock and made every save a revision-CAS write. Landing both meant `runOneRacerAttempt`'s own two failure exits (budget unavailable, Racer call failed) needed to persist through `saveGameIfRevisionMatches` — the same primitive every other exit in the route already uses — rather than the blind `saveGame` the guard's original branch had (predating the CAS primitive entirely). That is the only substantive change from the original guard implementation; the detector itself (`lib/duplicateQuestionGuard.ts`), its normalization rule, and its retry-loop shape are untouched.

**Confirmed coexisting, not just each independently intact.** `test/v282Integration.test.ts` drives the merged route directly: a stale answer is still rejected with zero mutation and zero Racer call regardless of the guard's presence in the same code path (A), and an accepted answer that happens to be followed by a duplicate Racer candidate commits correctly, blocks the duplicate, lands a real non-duplicate replacement, and leaves `game.revision` coherent — advanced exactly once for the request, not once per guard retry attempt (B). Both of §53's and the guard's own full test suites (`test/turnIntegrityHotfix.test.ts`, `test/duplicateQuestionGuard.test.ts`) pass unmodified against the merged code.

**Scope, unchanged from the original guard ticket.** Exact-duplicate detection only — no semantic similarity, no logical-redundancy or evidence-contradiction checking, no Guess Detector changes, no Hole ontology or target-grain work, no D9, no adjudication or IS-IS change, no Grok latency work. `racer/4.0.0` remains byte-for-byte unchanged.

### 55 V2.8.7 — GPT-6 Astra as the public Racer, Claude Fable 5.1 for adjudication, per-game AI cost

**What changed, in one line.** The one public Racer moves from Grok (`xai`) to OpenAI `gpt-6-astra` at `reasoning.effort: "low"`; the two adjudication seats (Adjudicator, Integrity Review) move from `claude-sonnet-5` to `claude-fable-5-1` at `output_config.effort: "low"`; every billable model call a game makes is now recorded with its token usage so cost can be read per game and per seat. `racer/5.0.0`, the Layer Two engine and its validation, Phase One, the V2.8.6 lock/CAS and authorization work, and the Validator/Composer seats are untouched. Public branding stays "Barkóba AI"; nothing player-facing names a vendor or model.

**The Racer (`lib/providers/openai.ts`).** A third transport behind the V2.5-B2 provider boundary, registered exactly as `xai` was — together with its adapter, so the exhaustive registry cannot compile without it — and selected by `PUBLIC_RACER_PROVIDER = "openai"` in `app/api/game/create/route.ts` under the same server-authoritative rule (an ordinary caller's `racer_provider` is never read; an unavailable provider refuses the game, never substitutes). Verified against OpenAI's official model page, latest-model guide, function-calling and reasoning guides on 2026-09-05: tool calling for this model requires the Responses API, so the adapter speaks `/v1/responses` — `instructions` + `input`, a FLAT function tool forced by name, `parallel_tool_calls: false`, `reasoning: {effort}`, `max_output_tokens`, `store: false`, and no sampling parameters at all. `strict` is sent as `false` on purpose: strict mode would require rewriting the Racer's schema per vendor, which is exactly the normalisation the boundary forbids, and the engine already validates every returned action and Layer Two field. Effort is server-held configuration for this provider (`OPENAI_REASONING_EFFORT_RACER`, default `low`) rather than "unset means provider default" as for `xai`, because the V2.8.7 decision is a specific effort, not whatever the vendor defaults to; the turn loop itself still never sets effort (`test/xaiProvider.test.ts` keeps asserting that). The returned function call's name is checked against the forced tool, its arguments must parse to an object, and `status: "incomplete"` with `max_output_tokens` is logged with the cap to raise.

**Adjudication (`lib/providers/anthropic.ts`, `lib/prompts/adjudicator.ts`, `lib/prompts/integrityReview.ts`).** The blocker that made this more than a model-id swap: Claude Fable 5.1 returns 400 for forced tool choice (`tool_choice: type "tool" and "any" are not supported for this model`), and every judgment call in this codebase forces its tool. Anthropic's documented migration — `tool_choice: auto`, `strict: true` with `additionalProperties: false`, and an explicit instruction that the tool call is required — is implemented as a runtime self-heal with the same shape as the existing sampling-parameter rule: the request goes out in forced mode, byte-identical to before; on that specific 400 it is retried once in auto+strict mode with a RESPONSE CONTRACT paragraph appended to the system prompt; the model is remembered and later calls skip the wasted round trip. Nothing in the judgment prompts themselves changed. In both modes the returned `tool_use` block must carry the expected tool name and an object input; `runAdjudicator` refuses a verdict outside `{correct, incorrect}` or one without reasoning, and `runIntegrityReview` refuses a verdict outside `{upheld, violated}` or a non-array `contradicting_turns` — each an explicit `*InvalidOutputError`, never a coerced verdict, never an automatic win, and (for Integrity Review) deliberately NOT the `IntegrityReviewIncompleteError` that earns the one bounded retry. `stop_reason: "refusal"` throws `AnthropicRefusalError`: no substitute model, no automatic retry, no inferred verdict; the resolve route surfaces the same `adjudicator_unavailable` / `integrity_review_unavailable` 502s and localized messages as before, the game stays in `resolving`, and the refusal is recorded (with whatever usage was billed) as its own telemetry status. Two account-level facts remain outside this code: Fable 5.1 requires 30-day data retention on the Anthropic organisation (ZDR organisations get 400 on every request), and model access itself is verified on a Preview deployment, not here.

**Cost (`migrations/0013_ai_usage_telemetry.sql`, `lib/corpus/turnTelemetry.ts`, `lib/aiCost.ts`, `scripts/reportAiCost.ts`).** `corpus.turn_operations` gains the requested model id, the effort sent, the request mode, and five token columns (uncached input, cached input, cache-write input, output, reasoning) appended after the 0012 columns so every positional reader is unchanged. Every adapter now returns normalised usage; the Racer's guarded attempt row is completed with it, and the guess-intent sub-call, both adjudication seats (each bounded attempt separately), and the other Anthropic seats (Validator, Composer choice/answer/clue, question-edit judge) each write one terminal row through the same fail-open upsert. The create route mints the game id before its first model call so those calls are attributable. Three rules are deliberate and tested: unknown is not zero (a null token count or an unpriced model makes a call UNPRICED and a game "partially priced", never $0); no invented average (a per-game figure exists only for games whose every call was priced, reported for completed and abandoned cohorts separately); no double counting (one row per billable call, rejected duplicates and refusals included, reasoning tokens counted once inside output). Prices are dated observations in `AI_PRICE_LIST` — `gpt-6-astra` $10/$1/$50 per 1M (OpenAI pricing page, read 2026-09-05, standard tier), `claude-fable-5-1` $10/$0.25 cache read/$12.50 cache write/$50 — and a changed list price is a new dated entry, not an edit.

**Verified through the application path, hermetically.** `test/openaiProvider.test.ts` (request contract, response parsing, effort, cap, provenance, usage, isolation), `test/anthropicAdjudicationCompat.test.ts` (forced-mode byte-identity, the self-heal, per-model learning, tool-name and payload validation, refusal, both seats end to end through their real functions, the resolve route's preserved contracts), `test/aiCost.test.ts`, and the extended `test/turnTelemetry.test.ts`. What is NOT proven here, said plainly: that this account reaches `gpt-6-astra` and `claude-fable-5-1`, and what a real game costs — both need a live Preview run, which is the next step, not this one.
