...
### 55 V2.8.3 — public Racer seat: `grok-4.20-0309-reasoning` → `grok-4.20-0309-non-reasoning`

**A speed/playability promotion, not an intelligence claim.** Production median Racer turn latency under `grok-4.20-0309-reasoning` measured ~19–22s, with real turns up to 52s and recurring genuine 60-second Vercel timeouts on `/turn` (confirmed via `get_runtime_errors`, not inferred) — unacceptable for interactive public play. A calibration check (this session, direct xAI API response) confirmed `reasoning_effort` is not a usable lever at all: xAI's API rejects it outright for `grok-4.20-0309-reasoning` (`"does not support parameter reasoningEffort"`). A seat comparison against `grok-4.20-0309-non-reasoning`, using that seat's own already-existing real human-game evidence (Marcali, Hole, Black hole — 99 real turns, no replay needed) found median 2.0s, P95 4.0s, zero turns at or above 30s across all three games, zero timeouts. This promotes that seat. It does **not** claim the non-reasoning model reasons better, resolves ontology gaps, fixes redundancy, or is superior in any dimension but latency — see the same three games' own documented defects (Hole's residual/ontology checklist-collapse, Marcali's logical redundancy, Black hole's bad-branch persistence), none of which this release touches or claims to fix.

**Mechanism: environment-only, by design already in place.** `lib/env.ts`'s `xaiModelRacer()` reads `process.env.XAI_MODEL_RACER` with no hardcoded model string anywhere in the codebase — the exact same mechanism V2.8.0 used to move onto `grok-4.20-0309-reasoning` in the first place (§52's own note: "Reaching a specific model... is entirely a matter of what `XAI_MODEL_RACER` is set to in the deployment environment — a production configuration decision, out of scope for this code change"). This release changes nothing in that mechanism; it is the same environment-variable action, this time in the other direction. `PUBLIC_RACER_PROVIDER` stays `"xai"` — this is a model change within the same provider, not a provider change, and `resolveRacerProvider()`'s existing two-refusals-never-a-substitution contract is unaffected. `VERSION` and this note are the only content changes this release contains.

**What this release could and could not verify directly.** No tool available in this session can read or write Vercel project environment variables (Preview or Production) — the only Vercel-adjacent tools available are deployment/build/log inspection and raw file deploys, none of them environment configuration. Preview deployment of this release candidate therefore correctly shows `version: "2.8.3"` and exercises the unchanged, already-verified V2.8.1/V2.8.2 code paths under whatever model Preview's own `XAI_MODEL_RACER` happens to already be set to — it cannot itself demonstrate `/api/version` reporting `grok-4.20-0309-non-reasoning`, because that requires setting an environment variable this session has no access to set. The seat's actual behavior (latency, provenance capture, schema compliance) is not unverified, though — it is exactly what the 99-real-turn evidence above already is. Setting `XAI_MODEL_RACER=grok-4.20-0309-non-reasoning` for the desired environment(s) remains a deliberate, explicit action for whoever holds that access.
