...
### 53 V2.8.1 — exact-duplicate question pre-emission guard

**One deterministic reliability guard, not a Racer-intelligence change.** `racer/4.0.0` (`CORE_RACER_RULES` in `lib/prompts/racer.ts`) is byte-identical to what it was in V2.8.0. This release adds exactly one thing: `lib/duplicateQuestionGuard.ts`, wired into `app/api/game/[id]/turn/route.ts`, which blocks the Racer from emitting a question to the Composer whose exact text (after whitespace-collapse and case-fold only — no diacritic stripping, no stemming, no semantic comparison) matches a question already asked and answered earlier in the same game's main branch.

**What this does NOT claim to solve, stated explicitly so it is not read as more than it is.** Forensic review of three real human games played 2026-09-01 (Marcali, Hole, Black hole — see the forensic report and `test/fixtures/threeHumanGames.ts`, which pins the actual corpus strings) surfaced four distinct redundancy/evidence-discipline patterns in one game (Marcali) alone: exact-repeat questions (3 instances), questions logically implied by evidence already on the board (2), and candidate questions contradicting an established hard fact (2). This release closes only the first of those. Semantic/paraphrased-duplicate detection, logical-implication ("answer already known") checking, and evidence-contradiction checking are each a materially different, more complex mechanism, each still open, and none is touched here. Also untouched: the Guess Detector, the Hole ontology/target-grain question, D9 bad-branch recovery (already an independently closed experimental thread — `racer/4.1.0` was tried and REJECTed on the `claude/m4-ai-play-intelligence-1394d4` branch, not reopened by this release), adjudication, IS-IS handling, and Strategy Memory.

**Mechanism.** Before a Racer turn is appended to `qa_log`, its candidate question is checked against every question already asked-and-answered on the game's main branch (`lib/duplicateQuestionGuard.ts`'s `priorAskedQuestions` / `isDuplicateQuestion`). A duplicate is never persisted or sent to the Composer: the route regenerates via the same Racer pipeline, up to a bounded 3-attempt cap (`MAX_DUPLICATE_QUESTION_ATTEMPTS` in the route), re-checking each replacement with the identical guard. If every attempt within the cap is a duplicate, the route reuses the existing `racer_unavailable` non-emission path unchanged — the recorded Composer answer is preserved via `saveGame`, nothing is appended to `qa_log`, no concede is forced, and the Racer's final-guess entitlement is not consumed. No new gameplay outcome was invented to make this pass.

**Validated against the real corpus, not only synthetic fixtures.** `test/duplicateQuestionGuard.test.ts` replays the actual question sequences from all three 2026-09-01 games: catches all 3 known Marcali exact duplicates (turns 17/27, 19/21, 29/33) and nothing else; zero false fires across Black hole's 27 questions and Hole's 35; a genuinely different but similarly-worded question (Marcali's turn 31, "near a *Somogy* county seat" vs. turn 19/21's "near *a* county seat") is confirmed to never fire; the three named-identity guess-like questions (Kaposvár, Siófok, Marcali) are confirmed out of this guard's scope entirely. The retry loop itself (`runWithDuplicateQuestionGuard`) is exercised with a mock producer standing in for the real Racer call, proving: block → regenerate → re-check → accept; retry-cap exhaustion never leaks a duplicate; a non-duplicate-related attempt failure (e.g. the spend ceiling) stops the loop immediately rather than being retried as though it were a duplicate.
